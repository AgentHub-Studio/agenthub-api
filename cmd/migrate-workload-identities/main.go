// Command migrate-workload-identities backfills tenant-local Keycloak workload
// identities after the corresponding public schema migration has been applied.
package main
...
type workloadIdentityProvisioner interface {
	EnsureWorkloadIdentity(context.Context, string) (workloadidentity.Credential, error)
}

type tenantCredentialStore interface {
	Store(context.Context, string, workloadidentity.Credential) error
}
...
func main() {
	...
	provisioner := apikc.NewProvisioner(apikc.Config{
		BaseURL: cfg.KeycloakBaseURL, AdminUsername: cfg.KeycloakAdmin.AdminUsername, AdminPassword: cfg.KeycloakAdmin.AdminPassword,
		AdminClientID: cfg.KeycloakAdmin.AdminClientID, AdminRealm: cfg.KeycloakAdmin.AdminRealm, FrontendClient: cfg.KeycloakAdmin.FrontendClient,
		WorkloadClientID: cfg.MCPRuntimeClientID, WorkloadAudience: cfg.MCPRuntimeAudience,
	})
	store := workloadidentity.NewService(workloadidentity.NewRepository(pool), cfg.MCPRuntimeCredentialEncryptionKey)
