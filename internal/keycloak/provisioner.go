...
// ProvisionRealm creates a Keycloak realm for the tenant, its frontend client,
// its internal workload client and default realm roles. It returns the unique
// workload credential so the caller can persist it encrypted. It tolerates 409
// Conflict and recovers the existing workload secret for idempotency.
...
// EnsureWorkloadIdentity creates or repairs the tenant-local service account
// used by agenthub-api to call the MCP runtime. It is public so an authorized
// migration command can backfill realms created before this contract existed.
func (p *Provisioner) EnsureWorkloadIdentity(ctx context.Context, tenantID string) (workloadidentity.Credential, error) {
	...
	resp, err := p.adminRequest(ctx, http.MethodPost,
		fmt.Sprintf("/admin/realms/%s/clients", url.PathEscape(tenantID)), payload)
	if err != nil {
		return workloadidentity.Credential{}, err
	}
...
func (p *Provisioner) findClientInternalID(ctx context.Context, tenantID, clientID string) (string, error) {
	resp, err := p.adminRequest(ctx, http.MethodGet,
		fmt.Sprintf("/admin/realms/%s/clients?clientId=%s", url.PathEscape(tenantID), url.QueryEscape(clientID)), nil)
	if err != nil {
		return "", err
	}
...
func (p *Provisioner) getClientSecret(ctx context.Context, tenantID, internalID string) (string, error) {
	resp, err := p.adminRequest(ctx, http.MethodGet,
		fmt.Sprintf("/admin/realms/%s/clients/%s/client-secret", url.PathEscape(tenantID), url.PathEscape(internalID)), nil)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
...
func (p *Provisioner) ensureAudienceMapper(ctx context.Context, tenantID, internalID string) error {
	...
	for _, mapper := range mappers {
		...
		if mapper.ProtocolMapper != "oidc-audience-mapper" ||
			mapper.Config["included.custom.audience"] != p.cfg.WorkloadAudience ||
...
func (p *Provisioner) audienceMapperPayload() map[string]any {
	return map[string]any{
		"name":            workloadAudienceMapperName,
		"protocol":        "openid-connect",
		"protocolMapper":  "oidc-audience-mapper",
		"consentRequired": false,
		"config": map[string]string{
			"included.custom.audience":  p.cfg.WorkloadAudience,
...
func (p *Provisioner) createRealmRoles(ctx context.Context, tenantID string, roles []string) error {
	for _, role := range roles {
		...
		body, _ := io.ReadAll(resp.Body)
		if err := resp.Body.Close(); err != nil {
			return fmt.Errorf("role %q close response: %w", role, err)
		}
		if resp.StatusCode == http.StatusConflict {
