...
func (r *postgresRepository) FindByKnowledgeBase(ctx context.Context, kbID uuid.UUID, req pagination.PageRequest) ([]Document, int64, error) {
	...
	rows, err := conn.Query(ctx,
		`SELECT id, knowledge_base_id, file_name, content_type, status, storage_path, file_size, created_at, updated_at
		 FROM document
	...
	for rows.Next() {
		var d Document
		if err := rows.Scan(&d.ID, &d.KnowledgeBaseID, &d.FileName, &d.ContentType, &d.Status, &d.StoragePath, &d.FileSize, &d.CreatedAt, &d.UpdatedAt); err != nil {
...
func (r *postgresRepository) FindByID(ctx context.Context, id uuid.UUID) (Document, error) {
	...
	err = conn.QueryRow(ctx,
		...
		id,
	).Scan(&d.ID, &d.KnowledgeBaseID, &d.FileName, &d.ContentType, &d.Status, &d.StoragePath, &d.FileSize, &d.CreatedAt, &d.UpdatedAt)
	if err != nil {
...
func (r *postgresRepository) Create(ctx context.Context, d Document) (Document, error) {
	...
	_, err = conn.Exec(ctx,
		`INSERT INTO document (id, knowledge_base_id, file_name, content_type, status, storage_path, file_size, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
		d.ID, d.KnowledgeBaseID, d.FileName, d.ContentType, d.Status, d.StoragePath, d.FileSize, d.CreatedAt, d.UpdatedAt,
	)
	if err != nil {
...
func (r *postgresRepository) UpdateStatus(ctx context.Context, id uuid.UUID, status DocumentStatus) (Document, error) {
	...
	err = conn.QueryRow(ctx,
		`UPDATE document
		 ...
		 WHERE id = $3
		 RETURNING id, knowledge_base_id, file_name, content_type, status, storage_path, file_size, created_at, updated_at`,
		status, now, id,
	).Scan(&d.ID, &d.KnowledgeBaseID, &d.FileName, &d.ContentType, &d.Status, &d.StoragePath, &d.FileSize, &d.CreatedAt, &d.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
