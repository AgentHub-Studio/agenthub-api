...
func (r *Repository) ListAll(ctx context.Context, tenantID string, pr pagination.PageRequest) ([]PromptExperiment, int, error) {
...
func (r *Repository) GetResults(ctx context.Context, tenantID string, experimentID uuid.UUID, pr pagination.PageRequest) ([]ExperimentResult, int, error) {
	conn, err := database.AcquireWithTenant(ctx, r.pool, tenantID)
	if err != nil {
		return nil, 0, err
	}
