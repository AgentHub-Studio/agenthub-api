...
func TestBDD_PermissionHook(t *testing.T) {
	...
	t.Run("Scenario_RemoveHookRetainsRestOfChain", func(t *testing.T) {
		...
		c := NewPermissionHookChain(nil)
		require.NoError(t, c.Register(PermissionHook{Name: "a", Phase: PermissionHookBeforeEvaluate, Enabled: true, Handle: noopHookHandle}))
		require.NoError(t, c.Register(PermissionHook{Name: "b", Phase: PermissionHookBeforeEvaluate, Enabled: true, Handle: noopHookHandle}))
		require.NoError(t, c.Register(PermissionHook{Name: "c", Phase: PermissionHookBeforeEvaluate, Enabled: true, Handle: noopHookHandle}))
