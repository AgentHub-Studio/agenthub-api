...
// StreamResult holds the stream and metadata about which model served the request.
type StreamResult struct {
	Stream <-chan ai.StreamChunk
	// Model is the model that actually served the request. Empty means
	// the primary model was used (no fallback occurred).
	Model string
}

// retryStreamWithFallback tries the primary model, then falls back to alternative models
// on persistent (non-transient) failures. Returns a StreamResult that includes which
// model actually served the request (important for per-model usage tracking).
func retryStreamWithFallback(
	...
	if err == nil || len(fallbackModels) == 0 {
		return StreamResult{Stream: stream}, err
	}

	// Don't fallback for prompt_too_long — that needs compaction, not a different model.
	if isPromptTooLong(err) {
	...
	for _, fallback := range fallbackModels {
		...
		fbOpts := opts
		fbOpts.Model = fallback
		stream, err = retryStream(ctx, model, messages, fbOpts, maxAttempts, source)
		if err == nil {
			return StreamResult{Stream: stream, Model: fallback}, nil
		}
	}

	return StreamResult{}, fmt.Errorf("all models failed (primary + %d fallbacks): %w", len(fallbackModels), primaryErr)
}
