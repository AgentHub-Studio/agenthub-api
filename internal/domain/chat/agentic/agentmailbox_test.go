...
func TestAgentMailbox_SendAndReadUnread(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()

	msg := mb.SendAgent(sessionID, "agent-a", "agent-b", "found schema info")
...
func TestAgentMailbox_Broadcast(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()

	mb.SendAgent(sessionID, "agent-a", "*", "broadcast message")
...
func TestAgentMailbox_MultipleMessages(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()

	mb.SendAgent(sessionID, "a", "b", "msg1")
	mb.SendAgent(sessionID, "c", "b", "msg2")
	mb.SendAgent(sessionID, "a", "d", "msg3") // not for b
...
func TestAgentMailbox_PendingCount(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()
	...
	mb.SendAgent(sessionID, "a", "b", "msg1")
	mb.SendAgent(sessionID, "a", "b", "msg2")
	mb.SendAgent(sessionID, "a", "c", "not-for-b")
...
func TestAgentMailbox_Cleanup(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()
...
func TestAgentMailbox_IsolatedSessions(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	session1 := uuid.New()
	session2 := uuid.New()
...
func TestDrainAgentMailbox_NoPending(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()

	result := agentic.DrainAgentMailbox(mb, sessionID, "agent-b")
	assert.Empty(t, result)
}

func TestDrainAgentMailbox_NilMailbox(t *testing.T) {
	result := agentic.DrainAgentMailbox(nil, uuid.New(), "agent-b")
	assert.Empty(t, result)
}

func TestDrainAgentMailbox_WithMessages(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()

	mb.SendAgent(sessionID, "agent-a", "agent-b", "found the schema")
	mb.SendAgent(sessionID, "agent-c", "agent-b", "query results ready")
...
func TestAgentMailbox_ConcurrentAccess(t *testing.T) {
	mb := agentic.NewAgentMailbox()
	sessionID := uuid.New()
	...
	go func() {
		for i := 0; i < 100; i++ {
			mb.SendAgent(sessionID, "writer", "reader", "msg")
		}
		close(done)
	}()
