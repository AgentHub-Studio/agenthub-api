...
func (s *Service) RunSession(ctx context.Context, sessionID uuid.UUID, userMessage, tenantID string, overrides RunOverrides) (<-chan RunEvent, error) {
	return s.RunSessionWithAttachmentsAndOverrides(ctx, sessionID, userMessage, tenantID, nil, overrides)
}
...
func (s *Service) RunSessionWithAttachments(ctx context.Context, sessionID uuid.UUID, userMessage, tenantID string, rawAttachments json.RawMessage) (<-chan RunEvent, error) {
	return s.RunSessionWithAttachmentsAndOverrides(ctx, sessionID, userMessage, tenantID, rawAttachments, RunOverrides{})
}

// RunSessionWithAttachmentsAndOverrides runs a session with optional attachments and
// transient Agent Studio settings. Overrides are deliberately never persisted.
func (s *Service) RunSessionWithAttachmentsAndOverrides(ctx context.Context, sessionID uuid.UUID, userMessage, tenantID string, rawAttachments json.RawMessage, overrides RunOverrides) (<-chan RunEvent, error) {
	if s.runner == nil {
